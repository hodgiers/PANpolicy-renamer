...
# Read device groups from a text file
with open('device_groups.txt', 'r') as file:
    device_groups = [line.strip() for line in file.readlines()]

def parse_and_print_policies(data, output_file):
                validation_required = []
                ...
                if 'entry' in data.get('result', {}):
                    for policy in data['result']['entry']:
                        ...
                        # If the description is "No description", use the name instead
                        ...
                        output_file.write(f'rename device-group {device_group} pre-rulebase security rules {name} to "{description}"\n')
...
# For each device group, run the rest of the script
for device_group in device_groups:
    ...
    output_filename = f"{device_group}_policyrename.txt"
    with open(output_filename, 'w') as output_file:
        ...
        if 'entry' in data_pre.get('result', {}):
            parse_and_print_policies(data_pre, output_file)
        else:
        ...
        if 'entry' in data_post.get('result', {}):
            parse_and_print_policies(data_post, output_file)
        else:
